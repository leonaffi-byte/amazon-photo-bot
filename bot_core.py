...
class UserSession:
    ...
    # Annotated overlay bytes (generated by annotate_with_overlays); used as image source
    # in _render_product when available; None when overlay generation failed or not yet run
...
class BotCore:
    ...
    async def _search_and_render(
        ...
        if loading_msg_ref:
            try:
                await self.adapter.edit_text(
                    loading_msg_ref,
                    text="\U0001f1ee\U0001f1f1 " + fmt._esc(t("product_israel_checking", lang=lang)),
                )
            except Exception:
                pass
    ...
    async def handle_photo(
        ...
        image_bytes = await asyncio.to_thread(_compress_image, raw_bytes)
        ...
        if len(detected_products) > 1 and not context_hint:
            ...
            from image_annotator import annotate_with_overlays, annotate_products
            try:
                annotated_bytes = await asyncio.to_thread(
                    annotate_with_overlays, image_bytes, detected_products
                )
            except Exception as exc:
                logger.debug("annotate_with_overlays failed (non-fatal): %s", exc)
